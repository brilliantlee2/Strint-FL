...
Usage:
  bash run_all.sh \
    ...
    [--glycine-bin-dir /path/to/glycine/bin] \
...
Notes:
  ...
  2. --glycine-bin-dir prepends a directory to PATH before resolving the glycine
     executable. Use it when glycine is not already on your PATH.
  3. --barcode-list-10bp accepts a plain text/CSV/TSV/XLSX file with 10bp barcodes
     and expands them into 26bp 3p/5p whitelists before running Strint2.
  ...
  10. Stint2 keeps the Strint2 dual-end barcode model, but writes Sockeye-style
  ...
  13. --light-output skips large upstream FASTQ outputs that are not consumed by
     the current Stint2.1 downstream alignment-first path:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --whitelist-3p|--whitelist-5p)
      echo "[ERROR] Strint2.6 no longer accepts --whitelist-3p/--whitelist-5p. Use --barcode-list-10bp instead." >&2
      exit 1
      ;;
    ...
    --glycine-bin-dir) GLYCINE_BIN_DIR="$2"; shift 2 ;;
...
if [[ "${SKIP_GLYCINE}" -eq 0 ]]; then
  if [[ -n "${GLYCINE_BIN_DIR}" ]]; then
    if [[ ! -d "${GLYCINE_BIN_DIR}" ]]; then
      echo "[ERROR] Missing glycine bin directory: ${GLYCINE_BIN_DIR}" >&2
      exit 1
    fi
    export PATH="${GLYCINE_BIN_DIR}:$PATH"
  fi
  require_cmd glycine
  log "Using glycine executable: $(command -v glycine)"
fi
...
for script in prepare_read_tags.py add_cb_ur_tags.py assign_genes.py add_gene_tags.py cluster_umis_allbam.py cell_umi_gene_table.py gene_expression.py assign_transcripts.py isoform_expression.py rna_qc_metrics_mixed.py Saturation.py read_qc_summary.py build_report.py barnyard_qc.py generate_26bp_whitelists.py build_mixed_species_reference.py generate_knee_plots.py; do
...
if [[ -f "${SCRIPT_DIR}/Cargo.toml" ]] && command -v cargo >/dev/null 2>&1; then
  ...
  (
    cd "${SCRIPT_DIR}"
    cargo build --release
  ) || log "cargo build failed under StrintRust0.0.3.5, will fall back to bundled Python scripts where needed"
fi
...
step_start
log "Step 2/6: Strint2.1 dual-end barcode split, correction, merge, and cell assignment"
MAIN_ARGS=(
  "${FULL_LENGTH_FASTQ}"
...
run_stage strint-rust "${SCRIPT_DIR}/main.py" "${MAIN_ARGS[@]}" 2>&1 | tee "${UPSTREAM_LOG}"
KNEE_ARGS=(
...
python3 "${DOWNSTREAM_DIR}/generate_knee_plots.py" "${KNEE_ARGS[@]}" 2>&1 | tee -a "${UPSTREAM_LOG}"
step_end "Step 2/6"
...
if [[ ! -f rna_violin_plot.png ]]; then
  python3 "${DOWNSTREAM_DIR}/rna_violin_plot.py" \
...
if [[ "${SKIP_GLYCINE}" -eq 1 ]]; then
  ...
  BUILD_REPORT_ARGS+=(--glycine-stats "${GLYCINE_OUTDIR}/identifying_statistics.txt")
fi
