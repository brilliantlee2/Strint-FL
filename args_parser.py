...
def build_parser():
    parser = argparse.ArgumentParser(
        description=(
            "Strint2: full-length read barcode splitting, whitelist generation, "
    ...
    parser.add_argument("--out_dir", type=str, default="Strint2")
    ...
    parser.add_argument(
        ...
        default="fixed_seq",
        help=(
            "Barcode/UMI extraction mode. fixed_seq uses the current Strint2 dual-end "
    ...
    parser.add_argument("--max_ed", type=int, default=5)
    ...
    parser.add_argument("--TOP1_ALPHA", type=float, default=0.3)
...
def set_parser(argv=None):
    ...
    print(f"[Strint2] Collected {len(args.fastq_fns)} FASTQ files.")
    print(f"[Strint2] Output directory: {args.out_dir}")
    return args
