...
use strint_rust::pipeline::{run_pipeline, PipelineConfig};

#[derive(Debug, Parser)]
#[command(version, about = "Rust-first single-cell analyzer for StrintRust0.0.2")]
struct Cli {
    #[arg(required = true)]
    ...
    #[arg(long = "out_dir", default_value = "StrintRust")]
    out_dir: PathBuf,
    ...
    #[arg(long = "max_ed", default_value_t = 5)]
    max_ed: usize,
    ...
    #[arg(long = "TOP1_ALPHA", default_value_t = 0.3)]
    top1_alpha: f64,
...
fn main() -> Result<()> {
    ...
    if cli.barcode_extract_mode != "fixed_seq" {
        anyhow::bail!(
            "StrintRust 0.0.2 currently supports --barcode_extract_mode fixed_seq only"
        );
    }
    if cli.threads > 0 {
    ...
    println!(
        "\n[StrintRust] Total elapsed: {:.2}s",
        total_t0.elapsed().as_secs_f64()
    );
    Ok(())
}

fn print_summary(summary: &strint_rust::pipeline::PipelineSummary, out_dir: &std::path::Path) {
    println!("\n=== StrintRust 0.0.2 Summary ===");
